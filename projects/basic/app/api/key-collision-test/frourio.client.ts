...
export const fc_1f8i0zm = (option?: FrourioClientOption) => ({
  $url: $url(option),
  $build(req: Parameters<ReturnType<typeof methods>['$get']>[0] | null): [
    ...
    return [{ lowLevel: true, baseURL: option?.baseURL, dir: '/api/key-collision-test', ...rest }, () => fc_1f8i0zm(option).$get(req)];
  },
  ...methods(option),
});
...
export const $fc_1f8i0zm = (option?: FrourioClientOption) => ({
  ...
  $build(req: Parameters<ReturnType<typeof methods>['$get']>[0] | null): [
    ...
    return [{ lowLevel: false, baseURL: option?.baseURL, dir: '/api/key-collision-test', ...rest }, () => $fc_1f8i0zm(option).$get(req)];
  },
...
const $url = (option?: FrourioClientOption) => ({
  get(req: { query: z.infer<typeof frourioSpec.get.query> }): { isValid: true; data: string; reason?: undefined } | { isValid: false, data?: undefined; reason: z.ZodError } {
    ...
    return { isValid: true, data: `${option?.baseURL ?? ''}/api/key-collision-test?${searchParams.toString()}` };
  },
});
